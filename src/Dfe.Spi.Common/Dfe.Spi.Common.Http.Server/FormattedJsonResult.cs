...
{// Created this as having issues with .NET 3.1 and customising serialization settings:
    ...
    {
        private static readonly JsonSerializerSettings BuiltInDefaultSerializerSettings = new JsonSerializerSettings
        {
            ContractResolver = new CamelCasePropertyNamesContractResolver(),
            NullValueHandling = NullValueHandling.Ignore,
        };

        static FormattedJsonResult()
        {
            DefaultSerializerSettings = BuiltInDefaultSerializerSettings;
        }
        ...
        public FormattedJsonResult(object value, HttpStatusCode statusCode)
            : this(value, statusCode, DefaultSerializerSettings)
        {
        }
